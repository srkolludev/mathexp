...
function create(req, res) {
  return _news2.default.create(req.body).then(respondWithResult(res, 201)).catch(handleError(res));
}
...
function patch(req, res) {
  ...
  return _news2.default.findById(req.params.id).exec().then(handleEntityNotFound(res)).then(patchUpdates(req.body)).then(respondWithResult(res)).catch(handleError(res));
}

// Deletes a News from the DB
function destroy(req, res) {
  return _news2.default.findById(req.params.id).exec().then(handleEntityNotFound(res)).then(removeEntity(res)).catch(handleError(res));
}

//Custom search by category name
function customTitle(req, res) {
  //menuitem instead of menuitem name required
  _news2.default.find({ title: { $regex: req.body.title, $options: 'i' } }).exec(function (err, data) {
    if (err) {
